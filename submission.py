...
def _hyperparameter(hyperparameters, name, default):
    return getattr(hyperparameters, name, default)


def _refresh_policy_kwargs(hyperparameters):
    mode = PreconditionerUpdateMode.normalize(
        _hyperparameter(hyperparameters, "preconditioner_update_mode", "foam")
    )
    return {
        "preconditioner_update_mode": mode,
        "matrix_root_inv_threshold": float(
            _hyperparameter(hyperparameters, "matrix_root_inv_threshold", 0.5)
        ),
        "max_epsilon": float(
            _hyperparameter(hyperparameters, "max_epsilon", 3.0e-7)
        ),
        "diagonal_residual_threshold": float(
            _hyperparameter(hyperparameters, "diagonal_residual_threshold", 0.75)
        ),
        "profile_preconditioner": bool(
            _hyperparameter(hyperparameters, "profile_preconditioner", False)
        ),
    }
...
def init_optimizer_state(
    ...
    optimizer_state = {
        'optimizer': DistributedShampoo(
            ...
            **_refresh_policy_kwargs(hyperparameters),
        ),
    }
    ...
    if len(rws_adagrad_params) > 0:
        ...
        optimizer_state["rws_optimizer"] = DistributedShampoo(
            ...
            use_protected_eigh=True,
            track_root_inv_residuals=False,
            preconditioner_update_mode=PreconditionerUpdateMode.STALE,
            matrix_root_inv_threshold=0.0,
            max_epsilon=hyperparameters.epsilon,
        )
    else:
